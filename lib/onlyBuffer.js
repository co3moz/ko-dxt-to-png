...
const { decryptTextureData } = require('./decryption');
...
module.exports = function bufferDxt2Png(buffer) {
  return new Promise((resolve, reject) => {
    ...
    offset += 3; // skip the NTF magic
    let version = buffer.readUInt8(offset);
    offset += 1;
    ...
    let format = 'unknown';
    const d3dFormat = typeData.readUInt32LE(0);
    crop = decryptTextureData(version, width, height, d3dFormat, crop);
    ...
    if (typeData.toString().indexOf('DXT1') != -1) {
      ...
      for (let i = 0; i < crop.length; i += 2) {
        ...
        temp[k++] = Math.floor(((a >> 10) & 0b11111) * 255 / 31);
        temp[k++] = Math.floor(((a >> 5) & 0b11111) * 255 / 31);
        temp[k++] = Math.floor((a & 0b11111) * 255 / 31);
        temp[k++] = a >> 15 ? 255 : 0;
      }
      crop = temp;
      format = 'a1r5g5b5';
